...
public class Astar
{
    ...
    public List<Spot> CreatePath(Vector3Int[,] grid, Vector2Int start, Vector2Int end, bool allowDiagonalMovement = false)
    {
        ...
        for (int i = 0; i < columns; i++)
        {
            for (int j = 0; j < rows; j++)
            {
                Spots[i, j].AddNeighboors(Spots, i, j, allowDiagonalMovement);
        ...
        while (OpenSet.Count > 0)
        {
            ...
            for (int i = 0; i < neighboors.Count; i++)//look threw our current spots neighboors (current spot is the shortest F distance in openSet
            {
                ...
                if (!ClosedSet.Contains(n) && n.Height < 1)//Checks to make sure the neighboor of our current tile is not within closed set, and has a height of less than 1
                {
                    ...
                    {
                        n.H = Heuristic(n, End, allowDiagonalMovement);
                        n.F = n.G + n.H;
                        n.previous = current;
                    }
                }
    ...
    private int Heuristic(Spot a, Spot b, bool allowDiagonal)
    {
        if (!allowDiagonal)
        {
            //manhattan
...
public class Spot
{
    ...
    public void AddNeighboors(Spot[,] grid, int x, int y, bool allowDiagonal)
    {
